...
dati=read.csv('data_hierarchical_EN.csv', header=T)
N = dim(dati)[1]
...
levels(g)=c(1:M)
g=as.numeric(g)
XX=as.matrix(dati[,c(5:12)])#covariate scuola
...
output <- coda.samples(model = model,
                       ...
                       n.iter = nit,
                       thin = thin)
save.image(file='risultato_EN_0.001_big_iter.Rdata')
