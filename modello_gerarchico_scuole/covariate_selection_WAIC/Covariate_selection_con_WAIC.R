...
# i = studente nella j = scuola
# Y_ij =  theta0 + theta_1j +theta_2j * Z_j + gamma_ij * X_ij + err_ij
...
inits2 <- function(ngr,p){
  list(
    theta0 = 0.1,
    theta = matrix(0.1, nrow=2, ncol=ngr),
    tau2_theta = matrix(1/50, 2, ngr),
    gamma = matrix(0.1, p, ngr),
    tau2_gamma = matrix(1/50, p, ngr),
    tau2 = 1/100
  )
...
my_algorithm_LMM2 <- function(dati){
  ...
  model <- jags.model(file='one_school_LMM.bug',
                     data= list(Y = Y,
                                X = dati_studente,
                                Z = cov_scuola,
                                N = N, 
...
# Conclusioni ------------------------------------------
# Vagamente interessanti risultano le covariate school_size e student_teacher_ratio
# Esse migliorano il WAIC di almeno 60 punti (su 35000, quindi comunque poco)
# Le altre covariate danno esito impercettibile