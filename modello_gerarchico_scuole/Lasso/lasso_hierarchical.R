...
model = jags.model("lasso_hierarchical_ari_plus_no_inter_troncated_normal.bug",
                 data = data_JAGS_EN,
                 n.adapt = 60000,
                 inits = inits,
                 n.chains = 1) 
...
param <- c( "theta",'l2','prec_gamma')
#param <- c( "theta",'a1','a2','prec_gamma','gamma')
output <- coda.samples(model = model,
                       variable.names = param,
                       n.iter = nit,
                       thin = thin)
save.image(file='lasso_norm_big.Rdata')
