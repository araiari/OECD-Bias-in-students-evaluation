...
#cross validation tra modello sei e sette
load('risultato_modello_biv_uno.rdata')
quantili=as.matrix(summary(output)$quantiles)
coeff_uno=(quantili[1:14,3])
beta_mate_uno=coeff_uno[3:8]
beta_read_uno=coeff_uno[9:14]
err_uno=rep(0,N)
for (i in 1:N){
  
  mate=Y[i,1]
  read=Y[i,2]
  prev_mate=coeff_uno[1]+X[i,]%*%beta_mate_uno
  prev_read=coeff_uno[2]+X[i,]%*%beta_read_uno
  err_uno[i]=abs(mate-prev_mate)+abs(read-prev_read)
}
...
errori=c(mean(err_uno),mean(err_due),mean(err_tre),mean(err_quattro),mean(err_cinque),mean(err_sei),mean(err_sette),mean(err_otto),mean(err_nove))
