...
inits = function() {
  list(#theta0 = 0.0, 
    ...
    tau2_theta0=0.1, tau2_gamma0=rep(5,M),tau2_theta1=0.1,tau2_theta2=0.1,
    
    #a normal exp(0.1), a big exp(0.01), a small exp(10)
    .RNG.seed = 321, .RNG.name = 'base::Wichmann-Hill') 
}

model = jags.model("modello_hierarchical_biv_nostud.bug",
                   data = data_JAGS_EN,
                   n.adapt = 20000,
                   inits = inits,
                   n.chains = 1) 
...
param <- c( 'beta0',"beta1","beta2",'sigma1','sigma2','rho','gamma0')
...
output <- coda.samples(model = model,
                       ...
                       n.iter = nit,
                       thin = thin)
save.image(file='modello_biv_hier_completo_uno.Rdata')
...
output <- coda.samples(model = model,
                       ...
                       n.iter = nit,
                       thin = thin)
save.image(file='modello_biv_hier_completo_due.Rdata')
...
#param <- c( "beta",'a1','a2','prec_gamma')
#param <- c( "beta0","beta1","beta2",'sigma1','rho','gamma0')
param <- c('beta0', "beta1","beta2",'sigma1','sigma2','rho','gamma0','gamma1','gamma2')
#param <- c( 'beta0',"beta1","beta2",'sigma1','sigma2','rho')
#param <- c( "theta",'a1','a2','prec_gamma','gamma')
output <- coda.samples(model = model,
                       variable.names = param,
                       n.iter = nit,
                       thin = thin)
save.image(file='modello_biv_hier_completo_tre.Rdata')


#x11()
#plot(output,ask=T)
#dev.off()
#plot(output[[1]][,1:5],ask=T)
...
output <- coda.samples(model = model,
                       ...
                       n.iter = nit,
                       thin = thin)
save.image(file='modello_biv_hier_completo_quattro.Rdata')
...
output <- coda.samples(model = model,
                       ...
                       n.iter = nit,
                       thin = thin)
save.image(file='modello_biv_hier_completo_cinque.Rdata')
...
output <- coda.samples(model = model,
                       ...
                       n.iter = nit,
                       thin = thin)
save.image(file='modello_biv_hier_completo_sei.Rdata')


x11()
#plot(output,ask=T)
#dev.off()
plot(output[[1]][,7000:7281],ask=T)
