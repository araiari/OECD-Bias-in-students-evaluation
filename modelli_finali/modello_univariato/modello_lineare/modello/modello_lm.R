...
inits = function() {
  list(beta0 = 0.0, beta = rep(0,p), sigma=87, #a normal exp(0.1), a big exp(0.01), a small exp(10)
...
#Y=(Y-mean(Y))/sd(Y)
#XX=(dati[,c(2,3,5,6,7,8,11,13,14,15,16,17,18,20,21,23,24,25,26)]) #togiere la risposta e pv5lang, read, togliere le scjole e le varaibili escluse
XX=(dati[,c(2,5,7,8,13,14,17,18,20,21,23,24)])
XX=as.matrix(XX)
X=matrix(0,dim(XX)[1], dim(XX)[2])
for (i in 1:dim(XX)[1]){
  for(j in 1:dim(XX)[2]){
...
output <- coda.samples(model = model,
                       ...
                       n.iter = nit,
                       thin = thin)
save.image(file='LM4_no_mate.Rdata')
#save(output,file='EN_jags1_a_normal.dat') 
#load('EN_jags1_a_normal.dat')
#x11()
#plot(output,ask=T)
...
#Y=(Y-mean(Y))/sd(Y)
#XX=(dati[,c(2,3,5,6,7,8,11,13,14,15,16,17,18,20,21,23,24,25,26)]) #togiere la risposta e pv5lang, read, togliere le scjole e le varaibili escluse
XX=(dati[,c(2,5,7,8,13,14,15,18,20,21,23,24)])
XX=as.matrix(XX)
X=matrix(0,dim(XX)[1], dim(XX)[2])
for (i in 1:dim(XX)[1]){
  for(j in 1:dim(XX)[2]){
...
output <- coda.samples(model = model,
                       ...
                       n.iter = nit,
                       thin = thin)
save.image(file='LM4_no_scie.Rdata')
#save(output,file='EN_jags1_a_normal.dat') 
#load('EN_jags1_a_normal.dat')
x11()
plot(output,ask=T)
